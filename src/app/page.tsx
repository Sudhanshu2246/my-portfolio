...
const TechStack = dynamic(
  () => import("@/components/tech-stack/TechStack"),
  { ssr: false }
);
...
function PanelShell() {
  ...
  const goToPanel = useCallback((index: number) => {
    ...
    if (target === currentRef.current && !isAnimating.current) return;
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const innerContainer = target.closest("[data-inner-scroll]") as HTMLElement;

      const delta = Math.abs(e.deltaX) > Math.abs(e.deltaY) ? e.deltaX : e.deltaY;

      if (innerContainer) {
        // Modals trap scroll completely — never switch panels
        if (innerContainer.hasAttribute("data-modal")) return;
      ...
      if (Math.abs(delta) < 3) return; // ignore tiny micro-movements
  ...
  useEffect(() => {
    ...
    const onTouchEnd = (e: TouchEvent) => {
      ...
      if (innerContainer) {
        ...
        if (dx > 0 && atLeft) allowPanelSwitch = true;
        else if (dx < 0 && atRight) allowPanelSwitch = true;
        else return;
      } else {
        allowPanelSwitch = true;
      }
  ...
  return (
    <PanelContext.Provider value={{ currentPanel, goToPanel }}>
