...
export default function Experience() {
    ...
    useEffect(() => {
        if (currentPanel !== 2 || animated.current)
            return;
    ...
    return (<div ref={panelRef} className="relative flex h-full w-full items-center justify-center overflow-hidden" style={{ background: "#0A0A0A" }}>
      ...
      <div className="pointer-events-none absolute inset-0" style={{
            background: "radial-gradient(ellipse 55% 65% at 65% 40%, rgba(0,229,168,0.06) 0%, transparent 70%)",
        }}/>
      ...
      <div className="relative z-10 flex h-full w-full max-w-[1400px] flex-col justify-center px-12 py-20 md:px-20 lg:px-32">
        ...
        <div className="mb-14">
          ...
          <div className="exp-line-h mt-8 h-px w-full" style={{
        ...
        <div className="space-y-10">
          {EXPERIENCE.map((exp) => (<div key={exp.company} className="exp-card group relative overflow-hidden rounded-2xl p-8" style={{
              ...
              <div className="pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100" style={{
                background: "radial-gradient(ellipse 70% 70% at 20% 50%, rgba(200,100,20,0.1) 0%, transparent 70%)",
            }}/>

              {/* Top row: Logo + period */}
              <div className="relative z-10 mb-6 flex flex-wrap items-center gap-4">
                {/* Real Dr Design logo */}
                <div className="flex h-10 w-auto items-center overflow-hidden rounded-lg px-2" style={{ background: "rgba(255,255,255,0.05)" }}>
                  <Image src="/logo-remastered.svg" alt="Dr Design logo" width={120} height={36} className="h-8 w-auto object-contain" unoptimized/>
                </div>

                <div className="rounded-full px-3 py-1 text-[11px] uppercase tracking-[0.3em]" style={{
              ...
              <div className="relative z-10 mb-5 grid grid-cols-1 gap-x-12 gap-y-2 md:grid-cols-2">
                {exp.highlights.map((h) => (<div key={h} className="flex items-start gap-2.5 text-[13px] leading-relaxed" style={{ color: "rgba(143,143,143,0.75)" }}>
                    <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: "#00E5A8" }}/>
                    {h}
                  </div>))}
              </div>
              ...
              <div className="relative z-10 flex flex-wrap gap-2">
                {exp.technologies.map((tech) => (<span key={tech} className="rounded-full px-3 py-1 text-[11px] uppercase tracking-wider" style={{
