...
export default function About() {
    ...
    useEffect(() => {
        if (currentPanel !== 1 || animated.current)
            return;
    ...
    return (<div ref={panelRef} className="relative flex h-full w-full items-center justify-center overflow-hidden" style={{ background: "#050505" }}>
      {/* Ambient glow */}
      <div className="pointer-events-none absolute inset-0" style={{
            background: "radial-gradient(ellipse 60% 70% at 30% 50%, rgba(200,100,20,0.08) 0%, transparent 70%)",
        }}/>
      <div className="pointer-events-none absolute inset-0" style={{
            background: "radial-gradient(ellipse 50% 60% at 75% 50%, rgba(0,229,168,0.05) 0%, transparent 70%)",
        }}/>
      ...
      <div className="relative z-10 grid h-full w-full max-w-[1400px] grid-cols-1 gap-16 px-12 py-20 md:grid-cols-2 md:px-20 lg:px-32">
        ...
        <div className="flex flex-col justify-center space-y-8">
          {/* Section label */}
          <div className="about-para text-[11px] font-semibold uppercase tracking-[0.35em]" style={{ color: "#C86414" }}>
            About Me
          </div>

          <h2 className="about-para font-display text-[42px] font-bold uppercase leading-[0.9] tracking-tight text-white md:text-[60px] lg:text-[72px]">
          ...
          {/* Divider */}
          <div className="about-para h-px" style={{
            width: "80px",
        ...
        <div className="flex flex-col justify-center">
          <div className="grid grid-cols-2 gap-8">
            {STATS.map((stat) => (<div key={stat.label} className="about-stat-num group relative overflow-hidden rounded-2xl p-8" style={{
                ...
                background: "radial-gradient(ellipse 80% 80% at 50% 50%, rgba(200,100,20,0.12) 0%, transparent 70%)",
            }}/>
                <div className="font-display text-[52px] font-bold leading-none md:text-[64px]" style={{
                background: "linear-gradient(120deg, #C86414 0%, #E8963C 40%, #00E5A8 100%)",
                ...
                <div className="mt-2 text-[12px] uppercase tracking-[0.25em]" style={{ color: "#8F8F8F" }}>
                  {stat.label}
                </div>
              </div>))}
          </div>
