...
export default function CustomCursor() {
    ...
    const pos = useRef({ x: typeof window !== "undefined" ? window.innerWidth / 2 : 0, y: typeof window !== "undefined" ? window.innerHeight / 2 : 0, sparkle: 1 });
    useEffect(() => {
        ...
        const handleMouseOver = (e) => {
            ...
            if (target.closest("a, button, [role='button'], input, select, textarea, [data-inner-scroll]")) {
                if (dotRef.current) gsap.to(dotRef.current, { scale: 1.5, backgroundColor: "#00E5A8", boxShadow: "0 0 15px #00E5A8", duration: 0.3 });
            }
        };
        const handleMouseOut = (e) => {
            const target = e.target;
            if (target.closest("a, button, [role='button'], input, select, textarea, [data-inner-scroll]")) {
                if (dotRef.current) gsap.to(dotRef.current, { scale: 1, backgroundColor: "#ffffff", boxShadow: "0 0 10px rgba(255,255,255,0.8)", duration: 0.3 });
            }
        };
    ...
    if (!isVisible)
        return null;
    return (<div ref={cursorRef} className="pointer-events-none fixed left-0 top-0 z-[9999] flex items-center justify-center mix-blend-screen" style={{ width: 0, height: 0 }}>
      {/* Flare (Rocket Exhaust) */}
      <div ref={flareRef} className="absolute" style={{
            width: "10px",
            height: "35px",
      ...
      {/* Dot (Rocket) */}
      <div ref={dotRef} className="absolute h-2.5 w-2.5 rounded-full bg-white shadow-[0_0_10px_rgba(255,255,255,0.8)]" style={{ willChange: "transform, background-color, box-shadow" }}/>
    </div>);
}
