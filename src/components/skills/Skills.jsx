...
export default function Skills() {
    ...
    useEffect(() => {
        if (currentPanel !== 5 || animated.current)
            return;
    ...
    return (<div ref={panelRef} className="relative flex h-full w-full items-center justify-center overflow-hidden" style={{ background: "#050505" }}>
      ...
      <div className="pointer-events-none absolute inset-0" style={{
            background: "radial-gradient(ellipse 55% 65% at 70% 50%, rgba(0,229,168,0.06) 0%, transparent 70%)",
        }}/>
      ...
      <div className="relative z-10 flex h-full w-full max-w-[1400px] flex-col justify-center px-12 py-20 md:px-20 lg:px-32">
        ...
        <div className="grid gap-7 md:grid-cols-2">
          {SKILLS.map((skill) => (<div key={skill.name} className="skill-row-item group">
              <div className="mb-2.5 flex items-center justify-between">
                <span className="font-display text-[17px] font-medium text-white md:text-[19px]">
                  {skill.name}
                </span>
                <span className="font-display text-[13px] font-bold" style={{ color: skill.color }}>
                  {skill.level}%
                </span>
              </div>
              <div className="h-[3px] overflow-hidden rounded-full" style={{ background: "rgba(255,255,255,0.06)" }}>
                <div className="skill-fill h-full rounded-full" style={{
                width: `${skill.level}%`,
