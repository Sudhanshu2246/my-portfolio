...
export default function CustomCursor() {
    ...
    if (!isDesktop)
        return null;
    return (<>
      <motion.div ref={ringRef} className="pointer-events-none fixed top-0 left-0 z-[9998] h-[40px] w-[40px] rounded-full border border-accent mix-blend-difference transition-[width,height,border-color] duration-300" style={{ x: springX, y: springY }}/>
      <motion.div ref={dotRef} className="pointer-events-none fixed top-0 left-0 z-[9999] h-[8px] w-[8px] rounded-full bg-accent mix-blend-difference transition-transform duration-200" style={{ x: springInnerX, y: springInnerY }}/>
    </>);
}
